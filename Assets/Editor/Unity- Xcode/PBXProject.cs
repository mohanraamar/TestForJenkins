    ...
    public class PBXProject
    {
        ...
        void RefreshMapsForGroupChildren(string projectPath, string realPath, PBXSourceTree realPathTree, PBXGroup parent)
        {
            ...
            foreach (string guid in children)
            {
                ...
                if (fileRef != null)
                {
                    pPath = Utils1.CombinePaths(projectPath, fileRef.name);
                    Utils1.CombinePaths(realPath, realPathTree, fileRef.path, fileRef.tree, out rPath, out rTree);
                ...
                if (gr != null)
                {
                    pPath = Utils1.CombinePaths(projectPath, gr.name);
                    Utils1.CombinePaths(realPath, realPathTree, gr.path, gr.tree, out rPath, out rTree);
        ...
        {
            return Utils1.CombinePaths(buildPath, "Unity-iPhone/project.pbxproj");
        }
        ...
        {
            path = Utils1.FixSlashesInPath(path);
            projectPath = Utils1.FixSlashesInPath(projectPath);
            ...
            if (guid == null)
            {
                PBXFileReference fileRef = PBXFileReference.CreateFromFile(path, Utils1.GetFilenameFromPath(projectPath), tree);
                PBXGroup parent = CreateSourceGroup(Utils1.GetDirectoryFromPath(projectPath));
        ...
        {
            sourceGroup = Utils1.FixSlashesInPath(sourceGroup);
        ...
        public void AddExternalProjectDependency(string path, string projectPath, PBXSourceTree sourceTree)
        {
            ...
            CreateSourceGroup(Utils1.GetDirectoryFromPath(projectPath)).children.AddGUID(fileRef.guid);
        ...
        */
        ...
        {
            PBXNativeTarget target = nativeTargets[targetGuid];
            filename = Utils1.FixSlashesInPath(filename);
            projectPath = Utils1.FixSlashesInPath(projectPath);
